 ...
 *
 * cc tags every system-injected user message — plan-mode reminders, denial
 ...
 * single-prompt agentic sessions (workflow runners, eval harnesses, the
 * deepagents `Agent` tool itself). Those sessions are ONE human input
 * but MANY API round-trips. cc switched to grouping by AIMessage `id`
 * change because each API response gets a unique id from Anthropic;
 * streaming chunks of the same response share the id so they stay in one
 * group.
 *
