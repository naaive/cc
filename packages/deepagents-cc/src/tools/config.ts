/**
 * Config tool — read or write whitelisted entries in `.claude/settings.json`.
 *
 * cc lets the model adjust a curated set of settings (model preference,
 * permission mode, output style, web-fetch hosts, etc.). Mutating arbitrary
 * keys would be a footgun, so the tool only sees a host-supplied
 * `supportedSettings` registry: each entry has a path, a type, optional
 * choice list, and a description shown to the model.
 *
 * Caveat: changes only affect the **on-disk** settings.json. The agent
 * was constructed with one snapshot of settings; live behaviour won't
 * change until the host re-creates the agent. The tool surfaces this in
 * its output so the model doesn't get confused.
 */
...
export interface ConfigSettingSpec {
  ...
  key: string
  /** Where this setting lives. user → ~/.claude/settings.json, project → repo. */
...
export interface ConfigToolOptions {
  ...
  cwd: string
  /** Override the user dir (default ~/.claude). */
  userDir?: string
}
...
export function createConfigTool(options: ConfigToolOptions) {
  ...
  const description = `Get or set a whitelisted Claude Code configuration setting.
...
function settingsPathFor(scope: 'user' | 'project' | 'local', options: ConfigToolOptions): string {
  if (scope === 'user') {
    const dir = options.userDir ?? path.join(process.env['HOME'] ?? '/tmp', '.claude')
