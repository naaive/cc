 ...
 *       Read with file_path=ccx-store://xxx to retrieve]"
 *   3. The model can pull the full payload back on demand via a virtual
 *      `ccx-store://<id>` path that Read recognises.
 *
 ...
 *
 * cc's `TOOLS_EXCLUDED_FROM_EVICTION`:
 ...
 * stashed into the store and replaced with a stub. cc uses an approximate
...
export const STORAGE_SCHEME = 'ccx-store://'
