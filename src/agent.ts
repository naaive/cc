/**
 * `createForgeAgent` — public entrypoint of the harness.
 *
 * This file owns:
...
export interface CreateForgeAgentParams {
  model?: string | LanguageModelLike
  /** User-supplied tools. Names must not collide with built-ins. */
  ...
  hooks?: HookConfig
  /** Extra reminders appended after the defaults. */
  reminders?: Reminder[]
  systemPromptAppendix?: string
  /** Use the embedded-agent identity prefix instead of the default identity. */
  embedded?: boolean
...
export interface ForgeAgentBundle {
  ...
  /** Loaded custom commands (from `.forge/commands/`). */
...
export function createForgeAgent(
  ...
  const projectMemory = params.skipProjectMemory ? [] : loadProjectMemory({ cwd })
  ...
  const { ccTools, allToolNames } = buildTools({
    ...
    subAgentFactory: sub => {
      ...
      const subBundle = createForgeAgent({
        ...params,
        cwd,
        subagents: [],
        fileStateCache,
