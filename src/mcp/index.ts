 ...
 *      langchain tools in one shot, ready to feed `createForgeAgent`
 *      via its `deferredTools` parameter.
 *   2. A `mcpToolName(server, tool)` namespace helper matching
 ...
 * `StructuredTool[]`. Pass them via `createForgeAgent({ deferredTools })`.
 *
 * Example:
 * ```ts
 ...
 * })
 * const { agent } = createForgeAgent({ deferredTools: mcp.tools })
