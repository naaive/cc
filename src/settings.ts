/**
 * Settings loader — `.forge/settings.json` hierarchy.
 *
 * Precedence (later overrides earlier):
 *   1. `~/.forge/settings.json`              (user)
 *   2. `<projectRoot>/.forge/settings.json`  (project)
 *   3. `<projectRoot>/.forge/settings.local.json` (local, gitignored)
 *
 * JSON parse failures are warnings, not hard errors: the harness should always
 * boot, even when the user's config is malformed.
 */
...
export function loadSettings(options: LoadSettingsOptions = {}): LoadedSettings {
  ...
  const userDir = options.userDir ?? path.join(os.homedir(), '.forge')
  ...
  const candidates: Array<{
    ...
    { scope: 'project', path: path.join(projectRoot, '.forge', 'settings.json') },
    { scope: 'local', path: path.join(projectRoot, '.forge', 'settings.local.json') },
  ]
