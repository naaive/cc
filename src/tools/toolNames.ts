 ...
 * where possible. We keep the original wording because the model's
...
/** The harness's classification of read-only vs. write tools. Used by plan mode. */
export const READ_ONLY_TOOL_SET: ReadonlySet<ToolName> = new Set([
  'Read',
  'Glob',
  'Grep',
  'WebFetch',
...
export const WRITE_TOOL_SET: ReadonlySet<ToolName> = new Set([
  'Bash',
  'KillShell',
  'Write',
  'Edit',
...
/**
* Our tool descriptions (as of v1.11.x). These are passed straight
...
You should only call this tool once you have written your plan and are ready to execute it. The user must approve the plan before you start making changes.`,
  ...
  Config: `Get or set a whitelisted configuration setting. Pass only \`key\` to read, or \`key\` + \`value\` to write. Writes are NOT hot-reloaded — the running agent keeps its old config until re-created.`,

  SlashCommand: `Invoke a user-defined custom command from .forge/commands/. Returns the EXPANDED prompt template (with $ARGUMENTS substituted, !bash inlined, @file inlined). Treat the result as instructions you should follow. Use DiscoverSlashCommands first if you don't know what's available.`,

  DiscoverSlashCommands: `List user-defined custom commands installed in .forge/commands/. Returns name + description for each.`,
